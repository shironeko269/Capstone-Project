function validateForm() {
    if (validateFormName() == false || validateFormAddress() == false || validateFormPhone() == false){
        return false;
    }
    takeOrder();

    return true;
}

function validateFormPhone() {
    var phoneno = /((09|03|07|08|05)+([0-9]{8})\b)/g;
    if(document.forms["orderform"]["phone"].value.match(phoneno)) {
        $("#phone").css("border-color", 'green');
        return true;
    }
    else {
        $("#phone").css("border-color", 'red');
        swal("Số điện toại không hợp lệ!", "error");
        return false;
    }
}
function validateFormAddress() {
    var specialChars = "<>@!#$%^&*()_+[]{}?:;|'\"\\/~`-=";
    var checkForSpecialChar = function(string){
        for(i = 0; i < specialChars.length;i++){
            if(string.indexOf(specialChars[i]) > -1){
                return true;
            }
        }

        return false;
    }
    if(checkForSpecialChar(document.forms["orderform"]["address"].value)){
        $("#address").css("border-color", 'red');
        swal("Địa chỉ không được tồn tại kí tự đặt biệt!", "error");
        return false;
    }
    if (document.forms["orderform"]["address"].value.length < 5 ) {
        $("#address").css("border-color", 'red');
        swal("Địa chỉ không được ngắn hơn 5 kí tự!", "error");
        return false;
    }else{
        $("#address").css("border-color", 'green');
        return true;
    }
    if (document.forms["orderform"]["address"].value == "" ) {
        $("#address").css("border-color", 'red');
        swal("Địa chỉ không được bỏ trống!", "error");
        return false;
    }else{
        $("#address").css("border-color", 'green');
        return true;
    }
}
function validateFormName() {
    if (document.forms["orderform"]["username"].value == "" ) {
        $("#username").css("border-color", 'red');
        swal("Họ và Tên không được bỏ trống!", "error");
        return false;
    }
    else if (document.forms["orderform"]["username"].value.length < 5) {
        $("#username").css("border-color", 'red');
        swal("Họ và Tên có độ dài ít nhất là 5 kí tự!", "error");
        return false;
    }
    else if (document.forms["orderform"]["username"].value.length > 30) {
        $("#username").css("border-color", 'red');
        swal("Họ và Tên có độ dài không vượt quá 30 kí tự!", "error");
        return false;
    }
    var specialChars = "<>@!#$%^&*()_+[]{}?:;|'\"\\,./~`-=";
    var checkForSpecialChar = function(string){
        for(i = 0; i < specialChars.length;i++){
            if(string.indexOf(specialChars[i]) > -1){
                return true;
            }
        }

        return false;
    }
    if(checkForSpecialChar(document.forms["orderform"]["username"].value)){
        $("#username").css("border-color", 'red');
        swal("Họ và Tên không được tồn tại kí tự đặt biệt!", "error");
        return false;
    }
    if (document.forms["orderform"]["username"].value != "" ) {
        $("#username").css("border-color", 'green');
        return true;
    }
    else if (document.forms["orderform"]["username"].value.length > 5 && document.forms["orderform"]["username"].value.length < 30) {
        $("#username").css("border-color", 'green');
        return true;
    }
}
function takeOrder() {
    var form = document.forms[0];
    var ProductId = [];
    var quantity = [];
    let totalPrice = 0.0;
    var myTab = document.getElementById('empTable');
    for (i = 0; i < myTab.rows.length; i++) {
        var objCells = myTab.rows.item(i).cells;
        ProductId.push(objCells.item(0).innerHTML);
        quantity.push(objCells.item(2).innerHTML);
        let temp = (objCells.item(7).innerHTML);
        totalPrice = (totalPrice + parseInt(temp,10));
    }
    var input1 = document.createElement('input');
    input1.setAttribute('name', 'ProductId');
    input1.setAttribute('value', ProductId);
    input1.setAttribute('type', 'hidden');
    form.appendChild(input1);
    var input2 = document.createElement('input');
    input2.setAttribute('name', 'quantity');
    input2.setAttribute('value', quantity);
    input2.setAttribute('type', 'hidden');
    form.appendChild(input2);
    var input3 = document.createElement('input');
    input3.setAttribute('name', 'totalPrice');
    input3.setAttribute('value', totalPrice);
    input3.setAttribute('type', 'hidden');
    form.appendChild(input3);
    form.submit();
    return;
}
const paymentStartProduct = () => {
    console.log("payment started....");
    if (validateFormName() == false || validateFormAddress() == false || validateFormPhone() == false){
        return false;
    }
    let totalPrice = 0.0;
    var myTab = document.getElementById('empTable');
    for (i = 0; i < myTab.rows.length; i++) {
        var objCells = myTab.rows.item(i).cells;
        let temp = (objCells.item(7).innerHTML);
        totalPrice = (totalPrice + parseInt(temp,10));
    }
    let amount = totalPrice;
    amount.toFixed(0);
    console.log(amount);
    if (amount == "" || amount == null) {
        swal("Failed !!", "Amount is required !!", "error");
        return;
    }
    //code
    //call ajax
    $.ajax(
        {
            url: '/user/create_order_product',
            data: JSON.stringify({amount: amount, info: 'order_request'}),
            contentType: 'application/json',
            type: 'POST',
            dataType: 'json',
            success: function (response) {
                console.log(response);
                if (response.status == "created") {
                    // open payment form
                    let options = {
                        key: "rzp_test_SYzQTN6MOUcF9L",
                        amount: response.amount,
                        currency: "INR",
                        name: "Yotsuba Payment",
                        description: "Donation",
                        image: "data:image/jpeg;base64,",
                        order_id: response.id,
                        handler: function (response) {
                            console.log(response.razorpay_payment_id)
                            console.log(response.razorpay_order_id)
                            console.log(response.razorpay_signature)
                            var form = document.forms[0];
                            let OrderPaymentId =response.razorpay_order_id;
                            var input4 = document.createElement('input');
                            input4.setAttribute('name', 'OrderPaymentId');
                            input4.setAttribute('value', OrderPaymentId);
                            input4.setAttribute('type', 'hidden');
                            form.appendChild(input4);
                            //alert("congrates !! Payment successful !!")
                            updatePaymentOnServerProduct(
                                response.razorpay_payment_id,
                                response.razorpay_order_id,
                                "Đã thanh toán!"
                            );
                        },
                        "prefill": {
                            "name": "",
                            "email": "",
                            "contact": ""
                        },
                        "notes": {
                            "address": "Razorpay Corporate Office"
                        },
                        "theme": {
                            "color": "#3399cc"
                        },
                    };

                    var rzp = new Razorpay(options);
                    rzp.on('payment.failed', function (response) {
                        console.log(response.error.code);
                        console.log(response.error.description);
                        console.log(response.error.source);
                        console.log(response.error.step);
                        console.log(response.error.reason);
                        console.log(response.error.metadata.order_id);
                        console.log(response.error.metadata.payment_id);
                        //alert("Oops payment failed!")
                        swal("Failed !!", "Oops payment failed !!", "error");
                    });
                    rzp.open();
                }
            },
            error: function (error) {
                console.log(error);
                alert("something went wrong!");
            }
        }
    )
};
function updatePaymentOnServerProduct(payment_id, order_id, status)
{
    $.ajax(
        {
            url: '/user/update_order_product',
            data: JSON.stringify({payment_id: payment_id, order_id: order_id, status: status}),
            contentType: 'application/json',
            type: 'POST',
            dataType: 'json',
            success:function (response) {
                swal("Xác nhận thanh toán!", "Chúc mừng bạn đã đặt hàng thành công!!", "success");
                setTimeout(() =>{
                    document.getElementById("inputOrder").click();
                },4000)
                sessionStorage.clear();
                // console.log(response)
                // if (response && response.userId != null){
                //     let user_id = parseInt(response.userId)
                //     setTimeout(() =>{
                //         window.location.href = `http://localhost:8080/user/history-invoice/${user_id}`;
                //     },4000)
                // }

            },
            error:function (error) {
                swal("Failed !!",
                    "Your payment is successful , but we did not get on server , we will contact you as soon as possible",
                    "error"
                );
            }
        })
}
